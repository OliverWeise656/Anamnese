...
function getAdditionalRecommendations(reason, age) {
    ...
    if (reason.includes('stimmstörung') || reason.includes('heiserkeit') || reason.includes('heiser') || reason.includes('rauhe stimme') || reason.includes('stimme') || state.reason.includes('stimmverlust') || state.reason.includes('phoniatrisches gutachten') || state.reason.includes('gutachten') || reason.includes('singen')) {
        state.voiceAnalysisRecommended = true;
...
function startHearingTestSequence() {
  // Sortiere die Wörter nach SNR, das größte SNR kommt zuerst
  testWords = audioFiles.sort((a, b) => b.snr - a.snr);
         ...
         // Reset text color to black (or any other default color you want)
            ...
            setTimeout(() => {
                if (state.voiceAnalysisRecommended) {
                    ...
                    setTimeout(() => {
                        alert('Herzlichen Dank für Ihre Unterstützung! Auf Ihrem Desktop wurde die folgende PDF abgelegt: ' + fileName + '. Bitte bringen Sie diese ausgedruckt zu Ihrem Besuch bei uns mit. Auf diese Weise tragen Sie zu einem reibungslosen Ablauf Ihres Besuchs bei uns bei! Bitte Beachten Sie, dass das Ergebnis des Hörtests besonderer Interpretation bedarf und nur für den Gebrauch in unserer Praxis vorgesehen ist.');
                    }, 5000);
                }
            }, 5000);
        });
